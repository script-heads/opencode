...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          const importResult = yield* Effect.promise(() =>
            Process.text([process.execPath, "import", sessionUrl], { nothrow: true }),
          )
    ...
    const next = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn([process.execPath, ...next], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`tunnelcode exited with code ${code}`))
  }),
})
